...
const mapOrderToLabTest = (order: LabOrder, templates: TestTemplate[], patients: Patient[]): LabTest => {
  ...
  return {
    ...
    completedBy: completedBy || 'Super Admin',
    discountAmount: order.discountAmount || 0,
    totalAmount: order.totalAmount,
    paidAmount: order.paidAmount,
    orderItems,
    createdAt: order.createdAt,
    createdBy: order.createdBy || order.updatedBy || 'Super Admin',
...
export function LabTestManagementNew({ hospital, userRole, currentUserId }: LabTestManagementNewProps) {
  ...
  const selectedTestsSubtotal = formData.selectedTests.reduce((sum, testId) => {
    const selectedTemplate = hospitalTests.find((template) => String(template.id) === String(testId));
    return sum + Number(selectedTemplate?.price || 0);
  }, 0);

  const selectedDiscountPercent = canEditLabOrderDiscount
    ? Math.min(Math.max(Number(formData.discountPercentage || 0), 0), 100)
    : 0;

  const selectedDiscountAmount = Math.min(
    Math.max((selectedTestsSubtotal * selectedDiscountPercent) / 100, 0),
    selectedTestsSubtotal
  );

  const selectedTestsPayableTotal = Math.max(0, selectedTestsSubtotal - selectedDiscountAmount);
  ...
  const filteredTestsForDropdown = hospitalTests.filter((test) => {
    const keyword = testSearchKeyword.trim().toLowerCase();
    if (!keyword) return true;
    const testName = String(test.testName || (test as any).test_name || '').toLowerCase();
    const testCode = String(test.testCode || (test as any).test_code || '').toLowerCase();
    const testType = String(test.testType || (test as any).test_type || '').toLowerCase();
    return testName.includes(keyword) || testCode.includes(keyword) || testType.includes(keyword);
  });
                        ...
                        filteredTestsForDropdown.map((test) => {
                          ...
                          return (
                            <button
                              ...
                              className={"w-full flex items-start gap-2 p-2 rounded text-left transition-colors " + (selected ? "bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800" : "hover:bg-gray-200 dark:hover:bg-gray-600 border border-transparent")}
                            >
            ...
            <div className="p-4 space-y-4">
              ...
              <div className="bg-white dark:bg-gray-700/30 rounded-lg p-3 border border-gray-200 dark:border-gray-600 shadow-sm">
                ...
                <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                  ...
                  <div className="space-y-0.5">
                    <label className="text-[10px] font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Completed At</label>
                    <p className="text-xs text-gray-900 dark:text-white font-medium">
                      {selectedTest.reportedAt ? formatDate(selectedTest.reportedAt, currentHospital.timezone, currentHospital.calendarType) : '-'}
                    </p>
                  </div>
                </div>
              </div>
