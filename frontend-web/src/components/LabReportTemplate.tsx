...
import { buildVerificationUrl } from '../app/utils/verification';
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '24px', marginBottom: '24px', fontSize: '14px' }}>
        ...
        <div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
            ...
            <div style={{ display: 'flex' }}><span style={{ fontWeight: '500', width: '128px' }}>Sample Date:</span> <span>{test.sampleCollectedAt ? new Date(test.sampleCollectedAt).toLocaleDateString() : '-'}</span></div>
            <div style={{ display: 'flex' }}><span style={{ fontWeight: '500', width: '128px' }}>Report Date:</span> <span>{test.reportedAt ? new Date(test.reportedAt).toLocaleDateString() : '-'}</span></div>
