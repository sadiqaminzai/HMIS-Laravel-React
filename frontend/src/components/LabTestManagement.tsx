...
import { Beaker, Plus, Search, Clock, CheckCircle, XCircle, FileText, Printer, Trash2, X, ArrowUp, ArrowDown, ArrowUpDown, FileSpreadsheet, Eye, Pencil } from 'lucide-react';
...
export function LabTestManagement({ hospital, userRole, currentUserId }: LabTestManagementProps) {
  ...
  const handleAdd = () => {
    ...
    const newTest: LabTest = {
      ...
      testNumber: `LAB-${new Date().getFullYear()}-${String(labTests.length + 1).padStart(3, '0')}`,
  ...
  return (
    <div className="space-y-3">
      {/* Compact Header & Controls - IDENTICAL TO DOCTOR MANAGEMENT */}
      ...
      {/* Lab Tests Table - COMPACT & IDENTICAL TO DOCTOR MANAGEMENT */}
              ...
              {filteredLabTests.length === 0 ? (
                ...
                filteredLabTests.map((test) => (
                  <tr key={test.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors group">
                    ...
                    <td className="px-4 py-2">
                      <div>
                        <div className="text-xs font-bold text-gray-900 dark:text-white">{test.patientName}</div>
                    ...
                    <td className="px-4 py-2">
                      <div className="relative">
                        <button
                          ...
                          className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[10px] font-medium border ${getStatusColor(test.status)} ${
