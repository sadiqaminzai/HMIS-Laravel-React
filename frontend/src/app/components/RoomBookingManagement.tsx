...
export function RoomBookingManagement({ hospital, userRole }: RoomBookingManagementProps) {
  ...
  // One booking per press -- a slow save must not become two admissions.
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-2xl w-full max-w-3xl border border-gray-200 dark:border-gray-700 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={guard(submitForm)} className="p-5 grid grid-cols-12 gap-3">
              ...
              <div className="col-span-12 flex gap-2 pt-2 border-t border-gray-200 dark:border-gray-700">
                ...
                <button type="submit" disabled={saving} className="flex-1 px-3 py-1.5 bg-blue-600 text-white rounded hover:bg-blue-700 transition-colors font-medium text-xs disabled:opacity-60 disabled:cursor-not-allowed">{saving ? t('ui.saving') : editing ? t('ui.update') : t('ui.create')}</button>
              </div>
            </form>
          </div>
        </div>
