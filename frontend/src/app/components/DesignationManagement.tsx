...
export function DesignationManagement({ hospital, userRole }: DesignationManagementProps) {
  const { t } = useTranslation();
  const { designations, addDesignation, updateDesignation, deleteDesignation, loading } = useDesignations();
  ...
  const canAdd = hasPermission('add_designations') || hasPermission('manage_designations');
  const canEdit = hasPermission('edit_designations') || hasPermission('manage_designations');
  const canDelete = hasPermission('delete_designations') || hasPermission('manage_designations');
  ...
  const filtered = useMemo(
    () => scopedDesignations.filter((d) => {
      const search = searchTerm.toLowerCase();
      return (
        d.name.toLowerCase().includes(search) ||
        (d.description || '').toLowerCase().includes(search) ||
        (d.department?.name || '').toLowerCase().includes(search) ||
        d.status.toLowerCase().includes(search)
      );
    }),
    [scopedDesignations, searchTerm]
  );
  ...
  const openAdd = () => {
    if (!canAdd) {
      toast.warning('You are not authorized to add designations');
      return;
    }
  ...
  const openEdit = (id: string) => {
    if (!canEdit) {
      toast.warning('You are not authorized to edit designations');
      return;
    }

    const record = designations.find((d) => d.id === id);
    if (!record) return;
  ...
  const onDelete = async (id: string) => {
    if (!canDelete) {
      toast.warning('You are not authorized to delete designations');
      return;
    }
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        ...
        const created = await addDesignation({
          hospitalId: formData.hospitalId,
          departmentId: formData.departmentId || undefined,
          name: formData.name,
          description: formData.description || undefined,
    ...
    <div className="p-6 space-y-6 max-w-[1200px] mx-auto">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 pb-2">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">Manage job titles and role designations.</p>
        </div>
        <AddButton onClick={openAdd} label={'Add Designation'} />
      </div>
          ...
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
              {paginatedDesignations.map((designation) => (
                <tr key={designation.id} className="hover:bg-gray-50/50 dark:hover:bg-gray-700/40 transition-colors border-b border-gray-100 dark:border-gray-800/50">
                  <td className="px-3 py-2.5 text-xs font-medium font-medium text-gray-900 dark:text-white">{designation.name}</td>
                  <td className="px-3 py-2.5 text-xs font-medium text-gray-600 dark:text-gray-300">{designation.department?.name || '-'}</td>
                  <td className="px-3 py-2.5 text-xs font-medium">
                    <span className={`px-3 py-1.5 rounded-[8px] text-xs font-semibold ${designation.status === 'active'
                  ...
                  <td className="px-3 py-2.5 text-xs font-medium">
                    <div className="flex items-center justify-end gap-3">
                      <button
                        onClick={() => openEdit(designation.id)}
          ...
          <form onSubmit={onSubmit} className="bg-white dark:bg-gray-800 rounded-lg shadow-2xl w-full max-w-xl max-h-[85vh] overflow-y-auto border border-gray-200 dark:border-gray-700 flex flex-col">
            <div className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 px-3 py-2 flex items-center justify-between rounded-t-lg sticky top-0 z-10">
              <h2 className="text-sm font-bold text-gray-900 dark:text-white">
                {editingId ? 'Edit Designation' : t('ui.addDesignation')}
            ...
            <div className="p-3 space-y-2">
              ...
              <div className="space-y-4">
                <h3 className="text-sm font-semibold text-gray-900 dark:text-white border-b border-gray-100 dark:border-gray-700 pb-2">Designation Details</h3>
