...
export function OtherIncomeCategories({ hospital, userRole }: OtherIncomeCategoriesProps) {
  ...
  const [form, setForm] = useState({
    ...
    name: '',
    description: '',
    status: 'active' as OtherIncomeCategory['status'],
  });
  ...
  const handleOpenModal = (category?: OtherIncomeCategory) => {
    if (category) {
      setEditing(category);
      setForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(error?.response?.data?.message || 'Failed to save other income category');
    } finally {
      setIsSubmitting(false);
    }
  };
