...
import { Loader2, Printer, Receipt, RotateCcw, Search, Smile, Wallet, X } from 'lucide-react';
...
import {
  ...
  fetchDentalServices,
  payDentalReceipt,
  reverseDentalPayment,
  updateDentalReceipt,
} from '../api/dental';

interface DentalReceiptsProps {
  hospital: Hospital;
  userRole: UserRole;
}
 ...
 */
export function DentalReceipts({ hospital, userRole }: DentalReceiptsProps) {
  ...
  const [receipts, setReceipts] = useState<DentalReceiptApi[]>([]);
  const [loading, setLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState('');
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editing, setEditing] = useState<DentalReceiptApi | null>(null);
  const [form, setForm] = useState(emptyForm);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [busyId, setBusyId] = useState<number | null>(null);
  const [paying, setPaying] = useState<DentalReceiptApi | null>(null);
  const [method, setMethod] = useState('cash');
  const [activeTab, setActiveTab] = useState<'receipts' | 'types'>('receipts');
  const [types, setTypes] = useState<DentalServiceApi[]>([]);

  const canManage = hasPermission('manage_dental_receipts');
  const canView = hasPermission('view_dental_receipts') || canManage;
  ...
  const canViewTypes = hasPermission('view_dental_services')
    || hasPermission('manage_dental_services')
    || hasPermission('add_dental_services')
    || hasPermission('edit_dental_services')
    || hasPermission('delete_dental_services');
  ...
  const paperSize = getPrintPaperSize(currentHospital.id, 'dental_receipt');
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    try {
      const rows = await fetchDentalReceipts(scopedHospitalId ? { hospital_id: scopedHospitalId } : {});
      setReceipts(rows);
    } catch (error: any) {
      toast.error(error?.response?.data?.message || 'Failed to load dental receipts');
    } finally {
      setLoading(false);
    }
  }, [scopedHospitalId]);
  ...
  const loadTypes = useCallback(() => {
    ...
    // has to render.
    fetchDentalServices(scopedHospitalId ? { hospital_id: scopedHospitalId } : {})
      .then(setTypes)
      .catch(() => {
  ...
  const openModal = (row?: DentalReceiptApi) => {
    if (row) {
      ...
      setEditing(null);
      const seeded = getDefaultDiscounts(currentHospital.id).dental;
      setForm({
        ...emptyForm(),
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      if (editing) {
        await updateDentalReceipt(editing.id, payload);
        toast.success('dental receipt updated');
      } else {
        await createDentalReceipt(payload);
        toast.success('dental receipt created');
      }
      ...
      toast.error(first || error?.response?.data?.message || 'Failed to save dental receipt');
    } finally {
      setIsSubmitting(false);
    }
  };

  const remove = async (row: DentalReceiptApi) => {
    if (!window.confirm(`Delete dental receipt for ${row.patient?.name ?? 'this patient'}?`)) return;
    try {
      await deleteDentalReceipt(row.id);
      toast.success('dental receipt deleted');
      await loadData();
    } catch (error: any) {
      toast.error(error?.response?.data?.message || 'Failed to delete dental receipt');
    }
  };

  const payable = (row: DentalReceiptApi) => Number(row.net_amount ?? row.fee ?? 0);

  const takePayment = async (row: DentalReceiptApi) => {
    setBusyId(row.id);
    try {
      await payDentalReceipt(row.id, { paid_amount: payable(row), payment_method: method });
      setPaying(null);
      await loadData();
   ...
   */
  const printReceipt = (row: DentalReceiptApi) => {
...
<html>
  ...
  <body>
    ...
    <div class="title">Dental Receipt</div>
  ...
  if (!canView && !canCreate) {
    return (
      <p className="text-sm text-gray-500 dark:text-gray-400">
        You do not have permission to view dental receipts.
      </p>
    );
  }
    ...
    <div className="space-y-3">
      ...
      <div className="flex items-center gap-1 border-b border-gray-200 dark:border-gray-700">
        {([
          ...
          { key: 'types' as const, label: 'Dental Services', icon: <Smile className="w-3.5 h-3.5" />, allowed: canViewTypes },
      ...
      {activeTab === 'types' ? (
        <DentalServices hospital={hospital} userRole={userRole} />
      ) : (
      <>

      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h1 className="text-lg font-bold text-gray-900 dark:text-white">Dental Receipts</h1>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {canCreate && <AddButton onClick={() => openModal()} label="Add dental receipt" />}
        </div>
      </div>
      ...
      {outstanding > 0 && (
        <p className="rounded-md border border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-900/20 px-3 py-2 text-xs text-amber-700 dark:text-amber-400">
          {outstanding} dental receipt{outstanding === 1 ? "" : "s"} awaiting payment.
        </p>
      )}
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between px-5 py-3 border-b border-gray-200 dark:border-gray-700">
              <h3 className="text-sm font-bold text-gray-900 dark:text-white">
                {editing ? 'Edit Dental Receipt' : 'Add Dental Receipt'}
              </h3>
              <button
