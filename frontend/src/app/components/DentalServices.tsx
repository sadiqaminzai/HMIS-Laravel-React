...
/**
 * The dental service catalogue.
 *
 * The list starts empty and is entirely the hospital's own: an English service
 * name, a description in the hospital's own wording (typically Pashto), and a
 * price. A receipt picks a service from here and inherits its price, instead
 * of someone retyping both every time.
 */
export function DentalServices({ hospital, userRole }: DentalServicesProps) {
  ...
  const canManage = hasPermission('manage_dental_services');
  const canView = hasPermission('view_dental_services') || canManage;
  const canCreate = hasPermission('add_dental_services') || canManage;
  const canEdit = hasPermission('edit_dental_services') || canManage;
  const canDelete = hasPermission('delete_dental_services') || canManage;
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      setTypes(await fetchDentalServices(scopedHospitalId ? { hospital_id: scopedHospitalId } : {}));
    } catch {
      toast.error('Failed to load dental services');
    } finally {
      setLoading(false);
    }
  }, [scopedHospitalId]);
  ...
  const openModal = (row?: DentalServiceApi) => {
    if (row) {
      setEditing(row);
      setForm({
        name: row.name || '',
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      if (editing) {
        await updateDentalService(editing.id, payload);
        toast.success('dental service updated');
      } else {
        await createDentalService(payload);
        toast.success('dental service created');
      }
  ...
  const remove = async (row: DentalServiceApi) => {
    ...
    setBusyId(row.id);
    try {
      await deleteDentalService(row.id);
      toast.success('dental service deleted');
      load();
    } catch (error: any) {
  ...
  if (!canView && !canCreate) {
    return (
      <p className="text-sm text-gray-500 dark:text-gray-400">
        You do not have permission to view dental services.
      </p>
    );
  }
  ...
  return (
    <div className="space-y-3">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h1 className="text-lg font-bold text-gray-900 dark:text-white">Dental Services</h1>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {canCreate && <AddButton onClick={() => openModal()} label="Add dental service" />}
        </div>
      </div>
      ...
      >
        ...
        <DataTableBody>
          {loading ? (
            ...
            <TableEmpty colSpan={6} message="No dental services yet." />
          ) : (
          ...
          <div className="w-full max-w-lg bg-white dark:bg-gray-800 rounded-lg shadow-xl">
            <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200 dark:border-gray-700">
              <h2 className="text-sm font-semibold text-gray-900 dark:text-white">
                {editing ? 'Edit Dental Service' : 'Add Dental Service'}
              </h2>
              <button
                type="button"
