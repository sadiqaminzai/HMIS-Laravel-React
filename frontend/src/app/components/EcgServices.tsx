...
import { Loader2, Save, Search, Smile, X } from 'lucide-react';
...
/**
 * The ECG study catalogue.
 *
 * The list starts empty and is entirely the hospital's own: an English study
 * name ("Resting ECG", "Stress ECG", "Holter 24h"), a description in the
 * hospital's own wording (typically Pashto), and a price. A receipt picks a
 * study from here and inherits its price, instead of someone retyping both
 * every time.
 */
export function EcgServices({ hospital, userRole }: EcgServicesProps) {
  ...
  const [editing, setEditing] = useState<EcgServiceApi | null>(null);
  ...
  const [viewing, setViewing] = useState<EcgServiceApi | null>(null);

  const canManage = hasPermission('manage_ecg_services');
  const canView = hasPermission('view_ecg_services') || canManage;
  const canCreate = hasPermission('add_ecg_services') || canManage;
  const canEdit = hasPermission('edit_ecg_services') || canManage;
  const canDelete = hasPermission('delete_ecg_services') || canManage;
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      setTypes(await fetchEcgServices(scopedHospitalId ? { hospital_id: scopedHospitalId } : {}));
    } catch {
      toast.error('Failed to load ECG studies');
    } finally {
      setLoading(false);
    }
  }, [scopedHospitalId]);
  ...
  const openModal = (row?: EcgServiceApi) => {
    if (row) {
      setEditing(row);
      setForm({
        name: row.name || '',
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      if (editing) {
        await updateEcgService(editing.id, payload);
        toast.success('ECG study updated');
      } else {
        await createEcgService(payload);
        toast.success('ECG study created');
      }
  ...
  const remove = async (row: EcgServiceApi) => {
    ...
    setBusyId(row.id);
    try {
      await deleteEcgService(row.id);
      toast.success('ECG study deleted');
      load();
    } catch (error: any) {
  ...
  if (!canView && !canCreate) {
    return (
      <p className="text-sm text-gray-500 dark:text-gray-400">
        You do not have permission to view ECG studies.
      </p>
    );
  }
  ...
  return (
    <div className="space-y-3">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h1 className="text-lg font-bold text-gray-900 dark:text-white">ECG Services</h1>
          <p className="text-xs text-gray-600 dark:text-gray-400">
            The services this hospital offers, and what each one costs.
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {canCreate && <AddButton onClick={() => openModal()} label="Add ECG Study" />}
        </div>
      </div>
      ...
      >
        ...
        <DataTableHead>
          <Th align="center">#</Th>
          <Th sort={sort} field="name">Service</Th>
          <Th>Description</Th>
        ...
        <DataTableBody>
          {loading ? (
            ...
            <TableEmpty colSpan={6} message="No ECG studies yet." />
          ) : (
            pageRows.map((row, index) => (
              <Tr key={row.id}>
                ...
                <td className="px-4 py-2 max-w-xs truncate">
                  <span dir="auto" className="text-[10px] text-gray-600 dark:text-gray-400">
                    {row.description || '-'}
                  </span>
                </td>
          ...
          <div className="w-full max-w-lg bg-white dark:bg-gray-800 rounded-lg shadow-xl">
            <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200 dark:border-gray-700">
              <h2 className="text-sm font-semibold text-gray-900 dark:text-white">
                {editing ? 'Edit ECG Service' : 'Add ECG Service'}
              </h2>
              <button
                type="button"
            ...
            <form onSubmit={guard(submit)} className="p-4 space-y-3">
              <div className="grid grid-cols-12 gap-3">
                ...
                <div className="col-span-12">
                  ...
                  {/* Where the Pashto wording goes, and any per-variant price
                      note the hospital wants on record. dir="auto" so RTL text
                      lays out correctly without forcing it on Latin text. */}
                  <textarea
