<div class="container">
    <div class="row mb-1">
        @if(Auth::user()->hasPermissionTo('designation.add'))
        <div class="col-md-1">
            <button class="btn btn-outline-primary btn-icon-text btn-sm" title="Add Designation" wire:click="create" data-bs-toggle="modal" data-bs-target="#modal" wire:ignore>
                <i class="btn-icon-prepend" data-feather="plus-circle"></i>Add
            </button>
        </div>
        @endif

        <div class="col-md-8 text-start"><h3>Designation List</h3s></div>
        <div class="col-md-3">
        ...
        <table class="table table-bordered">
            <thead>
                <tr>
                    ...
                    <th>Staus</th>
                    <th>Created By</th>
                    @if(Auth::user()->can('designation.delete') || Auth::user()->can('designation.edit'))
                    <th>Action</th>
                    @endif
                </tr>
            </thead>
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <button wire:click="edit({{ $designation->id }})" class="btn btn-outline-warning btn-icon-text btn-sm" data-bs-toggle="modal" data-bs-target="#modal" title="Edit" wire:ignore>
                            <i class="btn-icon-prepend" data-feather="edit"></i>Edit
                        </button>
                        @endif
                        @if(Auth::user()->can('designation.delete'))
                        <button wire:click="delete({{ $designation->id }})" class="btn btn-outline-danger btn-icon-text btn-sm" title="Delete" wire:ignore>
