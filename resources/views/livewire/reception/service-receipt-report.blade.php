<div class="container">
    ...
    <div class="row mb-2">
        ...
        <div class="col-md-1">
            ...
            <select wire:model="searchById" class="form-control">
                ...
                @foreach($service_receipts as $services_receipt)
                <option value="{{ $services_receipt->id }}">{{ $services_receipt->id }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="col-md-2">
            ...
            <select wire:model="searchByPatient" class="form-control">
                ...
                <option value="{{ $service_receipt->patient->name }}">{{ $service_receipt->patient->name }}</option>
                @endforeach
            </select>
        </div>
        <!-- Search by Doctor -->
        <div class="col-md-2">
            ...
            <select wire:model="searchByDoctor" class="form-control">
                ...
                @foreach($service_receipts as $service_receipt)
                @if($service_receipt->employee)
                <option value="{{ $service_receipt->employee->first_name }} {{ $service_receipt->employee->last_name }}">
                    {{ $service_receipt->employee->first_name }} {{ $service_receipt->employee->last_name }}
                </option>
                @endif
                @endforeach
            </select>
    ...
    <!-- Table displaying Service Receipt details -->
        ...
        <table class="table table-bordered">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{ $service_receipt->total_amount }}</td>
                    <td>{{$service_receipt->discount_amount }}</td>
                    <td>{{ $service_receipt->receipt_date }}</td>
                    @if(Auth::user()->can('service.receipt.details'))
                    <td>
                        @if(Auth::user()->can('service.receipt.details'))
                        <button wire:click="showDetails({{ $service_receipt->id }})" class="btn btn-icon-text btn-sm btn-outline-success" data-bs-toggle="modal" data-bs-target="#detailsModal">
    ...
    <!-- Pagination Links -->
    @if(!$search) {{ $service_receipts->links() }} @endif
    ...
    <div wire:ignore.self class="modal fade" id="searchModal" tabindex="-1" role="dialog" aria-labelledby="searchModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-xl" role="document">
            <div class="modal-content">
                <!-- Header -->
                <div class="modal-header text-white">
                    <h5 class="modal-title" id="searchModalLabel">Service Receipt Details</h5>
                    ...
                    <div class="col-md-3 text-end">
                        <div class="btn-group" role="group">
                            ...
                            <button class="btn btn-outline-danger btn-icon-text btn-sm" wire:click="pdf">PDF</button>
                                <button class="btn btn-outline-secondary btn-icon-text btn-sm" onclick="window.open('{{ url('/reception/s-report-print') }}', '_blank')">Print</button>
                            </div>
                    </div>
                    @endif
                ...
                <div class="modal-body p-2" style="max-height: 400px; overflow-y: auto;">
                    <div class="table-responsive">
                        <table class="table table-bordered">
                            ...
                            <tbody>
                                @forelse($searchResults as $index => $service_receipt)
                                <tr>
                                    <td>{{ $index + 1 }}</td>
                                    <td>{{ $service_receipt->id }}</td>
                                    <td>{{ $service_receipt->patient->name }}</td>
                                    ...
                                    <td>{{ $service_receipt->total_amount }}</td>
                                    <td>{{ $service_receipt->discount_amount }}</td>
                                    <td>{{ $service_receipt->total_amount - $service_receipt->discount_amount }}</td>
                                    <td>{{ $service_receipt->receipt_date }}</td>
                                </tr>
                                @empty
                                <tr>
                                    <td colspan="8" class="text-center">No Service Receipt records found.</td>
                                </tr>
                                @endforelse
                            </tbody>
                        </table>
                ...
                <div class="modal-footer d-flex justify-content-between" style="margin-left: 10%; margin-right: 10%;">
                    ...
                    <div>
                        <strong>Net Amount:</strong> <span>{{ number_format($totalNetAmount, 2) }}</span>
                    </div>
                </div>
            </div>
        </div>
