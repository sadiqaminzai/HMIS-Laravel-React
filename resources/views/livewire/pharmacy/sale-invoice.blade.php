<div class="container-fluid">
    <div class="container">
        <div class="card">
            <!-- Card Header: Sale Invoice -->
            <div class="card-header">
                <h4>Sale invoice</h4>
            </div>
            <div class="card-body">
                <form wire:submit.prevent="store">
                    <!-- Header Section for Sale Invoice Information -->
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group @error('patient_id') has-error @enderror">
                                    ...
                                    <select class="form-control form-control-sm"
                                        ...
                                        <option value="{{ $patient->id }}">{{ strtoupper($patient->name) }}</option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-md-2">
                                <div class="form-group @error('invoice_no') has-error @enderror">
                                    <input type="text" class="form-control form-control-sm" id="invoice_no" name="invoice_no" wire:model="invoice_no" placeholder="Invoice No" disabled>
                                    @error('invoice_no') <span class="text-danger">{{ $message }}</span> @enderror
                                </div>
                            </div>
                            <div class="col-md-2">
                                <div class="form-group @error('invoice_date') has-error @enderror">
                                    <input type="date" class="form-control form-control-sm" id="invoice_date" name="invoice_date" wire:model="invoice_date" @if(!$isEditing) disabled @endif>
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="row">
                                    ...
                                    <div class="col-md-9 justify-content-start">{{ $age }} year(s)</div>
                                </div>
                            </div>
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="row">
                                    ...
                                    <div class="col-md-9">{{ strtoupper($patient_address) }}</div>
                                </div>
                            </div>
                            <div class="col-md-4"></div>
                            <div class="col-md-2">
                    ...
                    <!-- Body Section for Sale Information -->
                    <div class="row mt-2">
                        <div class="col-md-12">
                            <div class="card">
                                <div class="card-body">
                                    <div class="table-responsive" style="min-height: 196px; max-height: 196px; overflow-x: auto; overflow-y: auto;">
                                        <table class="table table-bordered table-hover" style="border-spacing: 0; border-collapse: collapse;">
                                            <thead class="mb-2">
                                                <tr>
                                                    <th class="p-1" style="position: sticky; top: 0; z-index: 1; width: 25%;">Product Name</th>
                                                    <th class="p-1" style="position: sticky; top: 0; z-index: 1; width: 10%;">Sale Price</th>
                                                    <th class="p-1" style="position: sticky; top: 0; z-index: 1; width: 10%;">Batch No</th>
                                                    @if(!$isEditing || !$isChangesEnabled)
                                                    <th class="p-1" style="position: sticky; top: 0; z-index: 1; width: 10%;">Mfg. Date</th>
                                                    <th class="p-1" style="position: sticky; top: 0; z-index: 1; width: 10%;">Expiry</th>
                                                    @endif
                                                    @if($isEditing)
                                                    <th class="p-1" style="position: sticky; top: 0; z-index: 1; width: 10%;">Stock</th>
                                                    @endif
                                                    <th class="p-1" style="position: sticky; top: 0; z-index: 1; width: 10%;">Quantity</th>
                                                    <th class="p-1" style="position: sticky; top: 0; z-index: 1; width: 10%;">Discount</th>
                                                    <th class="p-1" style="position: sticky; top: 0; z-index: 1; width: 15%;">Amount (AFN)</th>
                                                    @if($isEditing)
                                                    <th class="p-1" style="position: sticky; top: 0; z-index: 1; width: 15%;">Action</th>
                                                    @endif
                                                </tr>
                                            </thead>

                                            <tbody>
                                                @foreach ($saleInvoiceDetails as $key => $saleInvoiceDetail)
                                                <tr>
                                                    <td>
                                                        <div class="form-group @error('patient_id') has-error @enderror">
                                                            @if($isChangesEnabled)
                                                            <div class="input-group">
                                                                <div class="dropdown-container" x-data="{ showSearch: false }" @product-selected.window="showSearch = false">
                                                                    <div class="d-flex">
                                                                        <button type="button" class="btn btn-sm btn-outline-secondary " x-on:click="showSearch = !showSearch" title="Toggle search mode">
                                                                        </button>
                                                                        <div x-show="!showSearch" style="width: 100%;">
                                                                            <select class="form-control form-control-sm"
                                                                                id="product_id.{{ $key }}"
                                                                                name="product_id.{{ $key }}"
                                                                                wire:model="saleInvoiceDetails.{{ $key }}.product_id"
                                                                                wire:change="fetchProductDetails({{ $key }})"
                                                                                onchange="focusNextField('quantity.{{ $key }}')">
                                                                                <option value="">--Select--</option>
                                                                                @foreach($products as $product)
                                                                                    <option value="{{ $product->id }}"
                                                                                        @if(in_array($product->id, $this->getSelectedProductIds())) disabled @endif>
                                                                        ...
                                                                        <div x-show="showSearch" class="position-relative" style="width: 100%;">
                                                                            <input type="text" 
                                                                                class="form-control form-control-sm"
                                                                                placeholder="Search products..."
                                                                                wire:model.debounce.300ms="searchQuery"
                                                                                wire:keyup="searchProducts({{ $key }})"
                                                                                autocomplete="off">
                                                                                
                                                                            @if($showSearchResults && count($searchResults) > 0)
                                                                                <div class="position-absolute  border rounded shadow-sm w-100 mt-1 z-index-dropdown" 
                                                                                    style="z-index: 1000; max-height: 300px; overflow-y: auto; min-width: 350px; left: 0; right: 0;">
                                                                                    @foreach($searchResults as $product)
                                                                                        <div class="p-2 border-bottom cursor-pointer hover-bg-light" 
                                                                                            wire:click="selectProduct({{ $key }}, {{ $product->id }})"
                                                                                            style="cursor: pointer; transition: background-color 0.2s ease;">
                                                                                            <div class="d-flex justify-content-between">
                                                                                                <strong class="text-truncate" style="max-width: 180px;" title="{{ strtoupper($product->name) }}">{{ strtoupper($product->name) }}</strong>
                                                                                                @if($product->company)
                                                                            ...
                                                                            @elseif($showSearchResults && strlen($searchQuery) >= 2 && count($searchResults) == 0)
                                                                                <div class="position-absolute bg-white border rounded shadow-sm w-100 mt-1 p-2">
                                                                                    No products found.
                                                                                </div>
                                                                            @endif
                                                                        </div>
                                                            ...
                                                            @else
                                                            @foreach($products as $product)
                                                            @if($product->id == ($saleInvoiceDetails[$key]['product_id'] ?? null))
                                                            <span title="{{ strtoupper($product->name) }}">{{ strtoupper($product->name) }}</span>
                                                            @endif
                                                            @endforeach
                                                            @endif
                                                        </div>
                                                    ...
                                                    <td>
                                                        ...
                                                        <input type="text" class="form-control form-control-sm"
                                                            id="unit_price.{{ $key }}"
                                                            name="unit_price.{{ $key }}"
                                                            wire:model.defer="saleInvoiceDetails.{{ $key }}.unit_price"
                                                            onkeydown="focusNextOnEnter(event, 'batch_no.{{ $key }}')"
                                                            disabled>
                                                        @else
                                                        @if(isset($saleInvoiceDetails[$key]['unit_price']))
                                                        {{ strtoupper($saleInvoiceDetails[$key]['unit_price']) }}
                                                        @endif
                                                        @endif
                                                    </td>
                                                    <td>
                                                        ...
                                                        <input type="text" class="form-control form-control-sm"
                                                            id="batch_no.{{ $key }}"
                                                            name="batch_no.{{ $key }}"
                                                            wire:model.defer="saleInvoiceDetails.{{ $key }}.batch_no"
                                                            onkeydown="focusNextOnEnter(event, 'quantity.{{ $key }}')"
                                                            disabled>
                                                        @else
                                                        @if(isset($saleInvoiceDetails[$key]['batch_no']))
                                                        {{ strtoupper($saleInvoiceDetails[$key]['batch_no']) }}
                                                        @endif
                                                        @endif
                                                    </td>
                                                    @if(!$isEditing || !$isChangesEnabled)
                                                    <td>
                                                        @if(isset($saleInvoiceDetails[$key]['mfg_date']))
                                                        {{ $saleInvoiceDetails[$key]['mfg_date'] }}
                                                        @endif
                                                    </td>
                                                    <td>
                                                        @if(isset($saleInvoiceDetails[$key]['expiry_date']))
                                                        {{ $saleInvoiceDetails[$key]['expiry_date'] }}
                                                        @endif
                                                    </td>
                                                    @endif
                                                    @if($isEditing)
                                                    <td>
                                                        @if($isChangesEnabled)
                                                        <input type="text" class="form-control form-control-sm"
                                                            wire:model.defer="saleInvoiceDetails.{{ $key }}.stock_quantity"
                                                            disabled>
                                                        @else
                                                        @if(isset($saleInvoiceDetails[$key]['stock_quantity']))
                                                        {{ $saleInvoiceDetails[$key]['stock_quantity'] }}
                                                        @endif
                                                        @endif
                                                    </td>
                                                    @endif
                                                    <td>
                                                        ...
                                                        id="quantity.{{ $key }}"
                                                        name="quantity.{{ $key }}"
                                                        wire:model.lazy="saleInvoiceDetails.{{ $key }}.quantity"
                                                        min="0"
                                                        max="{{ $saleInvoiceDetails[$key]['stock_quantity'] ?? 0 }}"
                                                        wire:blur="calculateItemAmount({{ $key }})"
                                                        oninput="this.value = Math.min(this.value, {{ $saleInvoiceDetails[$key]['stock_quantity'] ?? 0 }}); this.value = this.value.replace(/[^0-9]/g, '');"
                                                        autocomplete="off"
                                                    ...
                                                    <td>
                                                        ...
                                                        <input type="text" class="form-control form-control-sm"
                                                            id="discount.{{ $key }}"
                                                            name="discount.{{ $key }}"
                                                            wire:model.lazy="saleInvoiceDetails.{{ $key }}.discount"
                                                            min="0"
                                                            wire:blur="calculateItemAmount({{ $key }})"
                                                            oninput="this.value = this.value.replace(/[^0-9]/g, '');"
                                                            autocomplete="off">
                                                        @else
                                                        @if(isset($saleInvoiceDetails[$key]['discount']))
                                                        {{ $saleInvoiceDetails[$key]['discount'] }}
                                                        @endif
                                                        @endif
                                                    </td>
                                                    <td>
                                                        ...
                                                        <input type="number" class="form-control form-control-sm"
                                                            id="amount.{{ $key }}"
                                                            name="amount.{{ $key }}"
                                                            wire:model.defer="saleInvoiceDetails.{{ $key }}.amount"
                                                            disabled>
                                                        @else
                                                        @if(isset($saleInvoiceDetails[$key]['amount']))
                                                        {{ $saleInvoiceDetails[$key]['amount'] }}
                                                        @endif
                                                        @endif
                                                    </td>
                                                    @if($isEditing)
                                                    <td>
                                                        <div class="d-flex">
                                                            <button type="button" class="btn btn-outline-danger btn-xs me-1" wire:click="removeSaleInvoiceDetail({{ $key }})">Remove</button>
                                                            
                                                            @if($isChangesEnabled)
                                                            <button type="button" class="btn btn-outline-info btn-xs" 
                                                                wire:click="toggleProductDetails({{ $key }})" 
                                                                title="Show/Hide product details">
                                                                {{ isset($saleInvoiceDetails[$key]['showDetails']) && $saleInvoiceDetails[$key]['showDetails'] ? 'Hide Details' : 'Details' }}
                                                            </button>
                                                            @endif
                                                        </div>
                                                        
                                                        @if($isChangesEnabled && isset($saleInvoiceDetails[$key]['showDetails']) && $saleInvoiceDetails[$key]['showDetails'])
                                                        <div class="mt-2 product-details-panel p-2 border rounded">
                                                            <div class="row g-2">
                                                                <div class="col-12">
                                                                    ...
                                                                    <input type="date" class="form-control form-control-sm"
                                                                        id="mfg_date.{{ $key }}"
                                                                        name="mfg_date.{{ $key }}"
                                                                        wire:model.defer="saleInvoiceDetails.{{ $key }}.mfg_date"
                                                                        disabled>
                                                                </div>
                                                                <div class="col-12">
                                                                    ...
                                                                    <input type="date" class="form-control form-control-sm"
                                                                        ...
                                                                        wire:model.defer="saleInvoiceDetails.{{ $key }}.expiry_date"
                                                                        disabled>
                                                                </div>
                                                            </div>
                                                        </div>
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-2">
                                            <div class="form-group @error('paid_amount') has-error @enderror">
                                                ...
                                                <input type="number" class="form-control form-control-sm" id="paid_amount" name="paid_amount" wire:model="paid_amount" wire:blur="calculateDueAmount" placeholder="0.00" @if(!$isEditing) @endif disabled>
                                        ...
                                        <div class="col-md-2">
                                            ...
                                            <div class="btn-group btn-group-justified" role="group">
                                                @if($isEditing)
                                                @if(auth()->user()->can('pharmacy.saleinvoice.add'))
                                                ...
                                                @endif
                                                @if($isStoring)
                                                @if(auth()->user()->can('pharmacy.saleinvoice.cancel'))
                                                ...
                                                @endif
                                                @if(auth()->user()->can('pharmacy.saleinvoice.edit'))
        ...
        @if(!$isEditing)
        @if(
        auth()->user()->can('pharmacy.saleinvoice.add') ||
        auth()->user()->can('pharmacy.saleinvoice.edit') ||
        auth()->user()->can('pharmacy.saleinvoice.search') ||
        auth()->user()->can('pharmacy.saleinvoice.delete') ||
        auth()->user()->can('pharmacy.saleinvoice.navigate') ||
        auth()->user()->can('pharmacy.saleinvoice.view') ||
        auth()->user()->can('pharmacy.saleinvoice.print')
        )
        <div class="col-md-12">
            <div class="btn-group btn-group-justified" role="group" aria-label="Action Buttons" style="width: 100%;">
                ...
                @if(auth()->user()->can('pharmacy.saleinvoice.search'))
                ...
                @if(auth()->user()->can('pharmacy.saleinvoice.delete'))
                <button id="btn-delete" type="button" class="btn btn-outline-danger btn-icon-text btn-sm" onclick="confirmDelete({{ $invoice_no }})" wire:ignore>
                    <i class="btn-icon-prepend" data-feather="trash-2"></i>4.Delete
                </button>
                @endif

                @if(auth()->user()->can('pharmacy.saleinvoice.navigate'))
                <button id="btn-previous" type="button" class="btn btn-outline-secondary btn-icon-text btn-sm" wire:click.prevent="previous({{ $invoice_no }})" wire:ignore>
                    <i class="btn-icon-prepend" data-feather="arrow-left"></i>5.Previous
                </button>
                <button id="btn-next" type="button" class="btn btn-outline-secondary btn-icon-text btn-sm" wire:click.prevent="next({{ $invoice_no }})" wire:ignore>
                ...
                @if(auth()->user()->can('pharmacy.saleinvoice.print'))
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <div class="form-group">
                            <label for="purchase_info">Search Sale Invoice</label>
                            <select id="purchase_info" class="form-control" wire:change="fetchSelectedData($event.target.value)">
                                <option disabled selected>S.Inv No | patient Name | Sale Invoice Date</option>
                                @foreach ($showSearchData as $data)
                                <option value="{{ $data['invoice_no'] }}">
                                    {{ $data['invoice_no'] }} | {{ $data['patient_name'] }} | {{ $data['invoice_date'] }}
                                </option>
                                @endforeach
                            </select>
                        </div>
                ...
                <div class="modal-content" style="background-color: white; color: black;">
                    ...
                    <div class="modal-body" style="background-color: white; color: black; max-height: 80vh; overflow-y: auto;">
                        <div class="container" style="width: 100%; font-family: Arial, sans-serif; padding: 20px; border: 1px solid #000;">
                            ...
                            <div class="row mt-4">
                                <div class="col-6">
                                    <p>patient Name: <span style="font-size: 1.5em;">{{ strtoupper($previewData['patient_name'] ?? '') }}</span></p>
                            ...
                            <div class="table-responsive">
                                <table class="table table-bordered mt-3 text-dark">
                                    ...
                                    <tbody>
                                        @foreach ($previewData['saleInvoiceDetails'] ?? [] as $detail)
                                        <tr>
                            ...
                            <div class="row mt-4">
                                <div class="col-3">
                                    ...
                                    <p>No of item(s): {{ count($previewData['saleInvoiceDetails'] ?? []) }}</p>
                                </div>
        ...
        <script>
            ...
            function printPreview() {
                ...
                setTimeout(() => {
                    ...
                    printWindow.moveTo(0, 0); // Move the window to the top-left corner
                    printWindow.resizeTo(screen.width, screen.height); // Maximize the window
                        ...
                        <body>
                            <div class="container">
                                ...
                                <table class="table">
                                    ...
                                    <tbody>
                                        ${(previewContent.saleInvoiceDetails || []).map(detail => `
                                            <tr>
                                ...
                                <div class="totals" style="text-align: right;">
                                    <p>Total Discount: ${parseFloat(previewContent.total_discount || 0).toFixed(2)}</p>
                                    <p>Net Amount: ${parseFloat(previewContent.net_amount || 0).toFixed(2)}</p>
                                    <p>Paid Amount: ${parseFloat(previewContent.paid_amount || 0).toFixed(2)}</p>
                                    <p>Due Amount: ${parseFloat(previewContent.due_amount || 0).toFixed(2)}</p>
                                </div>
